...
class TweetsComponent extends React.Component {
  ...
  render() {
    ...
    let itemsCards = <TransitionGroup
      ...
      {items.map((x, i) => {
        return <CardComponent key={i} data={x} />
      }
      )}
    </TransitionGroup>;
    ...
    let searchControls =
      <div className="row">
        <input id="email" type="text" className="validate col-sm-12" value={this.state.searchTerm} onKeyPress={this.handleKeyPress} onChange={this.handleChange} />
    ...
    let filterControls = <div>
      ...
      <p>
        ...
        <label htmlFor="test5">Retweets Included</label>
      </p>
    </div>
